...
exports.handleNotification = async (req, res) => {
  ...
  // Now it only checks if the status is "2" (Success)
  if (status_code === "2") {
    await Payment.findOneAndUpdate(
      { appointmentId: order_id },
      { status: "SUCCESS", paidAt: Date.now() },
    );
    console.log(`✅ Test Success: Appointment ${order_id} marked as paid.`);
    return res.status(200).send("Verified (Test Mode)");
  }

  res.status(400).send("Payment Failed Status");
};
