        ...
        return (
          <div key={label} className="flex items-center">
            <div className="flex flex-col items-center">
              ...
              <span
                className={`text-xs mt-1 font-medium whitespace-nowrap ${active ? "text-green-dark" : done ? "text-green-primary" : "text-gray-400"}`}
              >
                {label}
              </span>
            </div>
            {i < steps.length - 1 && (
              <div
                className={`w-12 h-0.5 mb-5 mx-1 transition-colors ${step > num ? "bg-green-primary" : "bg-gray-200"}`}
              />
            )}
          </div>
        );
...
export default function PaymentPage() {
  ...
  const handlePaymentSuccess = async () => {
    try {
      ...
      alert(
        "Payment successful, but we couldn't update your appointment status. Please contact support.",
      );
      setStep(3);
    }
  };
    ...
    <div className="min-h-screen bg-green-light flex flex-col items-center justify-center p-6">
      ...
      {step === 1 && (
        <div className="bg-white rounded-2xl shadow-md p-8 w-full max-w-md animate-in fade-in slide-in-from-bottom-2 duration-300">
          ...
          <button
            onClick={() => setStep(2)}
            className="w-full bg-green-primary text-white font-bold py-4 rounded-2xl shadow-xl hover:bg-green-dark transition-all"
          >
            Proceed to Payment →
          </button>
        </div>
      ...
      {step === 2 && (
        <div className="w-full max-w-md animate-in fade-in zoom-in-95 duration-300">
          ...
          <button
            onClick={() => setStep(1)}
            className="w-full mt-4 text-xs text-gray-400 font-black uppercase text-center hover:text-gray-600"
          >
            ← Back
          </button>
        </div>
      ...
      {step === 3 && (
        <div className="bg-white rounded-2xl shadow-md p-8 text-center max-w-sm w-full animate-in zoom-in-95 duration-300">
          ...
          <p className="text-gray-500 text-sm mb-6">
            Payment completed and appointment confirmed.
          </p>
          <button
            onClick={() => navigate("/patient/appointments")}
            className="w-full bg-green-primary text-white font-semibold py-3 rounded-xl shadow-lg"
          >
